...
//! user/org the session is scoped to. Two well-known files by
//! convention:
//!
//! - `~/.smooth/auth/smooai-user.json` — Supabase user OAuth session
//! - `~/.smooth/auth/smooai.json` — M2M client_credentials session
//!
//! A single host can carry both simultaneously without collision.
//!
//! Permissions: files are created with mode 0600 so other host users
//! can't trivially scrape the access token.
...
/// One stored session — either Supabase user OAuth or M2M
/// `client_credentials`. The shape covers both because most fields
/// are optional.
...
pub struct Credentials {
    ...
    /// client-credentials, public Supabase one-shots) don't issue
    /// one or set its lifetime equal to access_token.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub refresh_token: Option<String>,
    /// When `access_token` expires. Used by SDK middleware to
    ...
    /// Active organization id, if the user picked one. Commands that
    /// take `--org` override this; commands that don't fall back to
    /// it.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// one expires without re-prompting the user. `None` for user
    /// OAuth flows.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// same 0600 file. Used for auto-refresh. `None` for user OAuth.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub client_secret: Option<String>,
    /// Kind discriminator so consumers can branch on which auth
    /// flow produced the session. Defaults to `M2m` for back-compat
    /// with existing `smooai.json` files that predate this field.
    #[serde(default)]
...
/// Which auth flow produced this session.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
#[serde(rename_all = "snake_case")]
pub enum CredentialKind {
    /// Service account via RFC 6749 `client_credentials` against
    /// `auth.smoo.ai`.
    #[default]
    M2m,
    /// Supabase user OAuth (localhost-callback flow).
    User,
}
...
/// Reads + writes the credentials file.
#[derive(Debug, Clone)]
...
impl CredentialsStore {
    /// Construct using the default path for **M2M** sessions
    /// (`~/.smooth/auth/smooai.json`). Honors `SMOOAI_AUTH_FILE`
    /// for test overrides.
    ///
    /// # Errors
    ...
    /// Construct using the default path for **user OAuth** sessions
    /// (`~/.smooth/auth/smooai-user.json`). Honors
    /// `SMOOAI_USER_AUTH_FILE` for test overrides.
    ///
    /// # Errors
    /// Fails when `$HOME` is unset *and* `SMOOAI_USER_AUTH_FILE` is
    /// not provided.
    ...
    ///
    /// # Errors
    /// IO failures (perms, EIO) or when the file exists but isn't
    /// valid JSON.
...
mod tests {
    ...
    #[test]
    fn default_m2m_and_user_paths_differ() {
        // Clear env overrides so we exercise the dirs_next path.
        let prev_m2m = std::env::var("SMOOAI_AUTH_FILE").ok();
        let prev_user = std::env::var("SMOOAI_USER_AUTH_FILE").ok();
        std::env::remove_var("SMOOAI_AUTH_FILE");
        std::env::remove_var("SMOOAI_USER_AUTH_FILE");
        ...
        // Restore env.
        if let Some(v) = prev_m2m {
            std::env::set_var("SMOOAI_AUTH_FILE", v);
        }
        if let Some(v) = prev_user {
            std::env::set_var("SMOOAI_USER_AUTH_FILE", v);
        }
    }

    #[test]
    fn credential_kind_defaults_to_m2m_for_back_compat() {
        // Existing smooai.json files predate the `kind` field; they
        // must deserialize as M2m.
        let raw = r#"{"access_token":"tok"}"#;
        let creds: Credentials = serde_json::from_str(raw).expect("parse");
        assert_eq!(creds.kind, CredentialKind::M2m);
    }
}
