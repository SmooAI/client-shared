//! RFC 6749 `client_credentials` grant against `auth.smoo.ai`.
//!
...
//! 4. Subsequent commands send `Authorization: Bearer <access_token>`.
//!
...
/// here. We default `expires_in` to 1 hour when missing.
...
/// own internal call sites set it to `"client_credentials"` for this
/// grant.
...
///
/// # Errors
/// Network failures, non-2xx responses, or missing/malformed
...
pub async fn client_credentials_grant(
    ...
    Ok(Credentials {
        ...
        // string so `whoami` shows *something* useful.
